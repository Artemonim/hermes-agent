...
def _(rid, params: dict, session: dict) -> dict:
    """Discriminated provider-limits envelope without exposing credentials.

    ``status`` is ``ok``, ``unsupported`` (stop refetching), or ``unavailable``
    (retryable, with ``reason``). Live-agent identity wins; persisted billing
    route is the fallback when no agent is built yet.
    """
    ...
    if agent is not None:
        ...
        # Non-str keys (lazy callables) are absent here — never str() them into a fake token.
    ...
    if not provider:
        return _ok(rid, {"status": "unavailable", "account_usage": None, "reason": "no-provider"})
    ...
    if provider == "openai-codex" and not api_key:
        return _ok(rid, {"status": "unavailable", "account_usage": None, "reason": "no-credential"})
    ...
    try:
        ...
        if snapshot is None or not snapshot.available:
            return _ok(rid, {"status": "unavailable", "account_usage": None, "reason": "no-data"})
        return _ok(rid, {"status": "ok", "account_usage": serialize_account_usage_snapshot(snapshot)})
    except Exception:
        logger.debug("failed to fetch session account usage", exc_info=True)
        return _ok(rid, {"status": "unavailable", "account_usage": None, "reason": "fetch-failed"})
