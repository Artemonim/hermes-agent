...
from tools.delegate_tool_config import (  # noqa: F401
    ...
    _apply_child_model_routing, _get_allow_model_override, _is_same_delegation_provider,
...
def _build_children(
    ...
    for i, t in enumerate(task_list):
        child_creds = per_task_creds[i] if i < len(per_task_creds) else per_task_creds[-1]
        overrides = {
...
def delegate_task(
    ...
    subagent_id: Optional[str] = None, message: Optional[str] = None, model: Optional[str] = None, parent_agent=None,
    ...
    if credentials_cfg is not None:
        ...
        for name in dict.fromkeys(n for n in override_names if n):
            ...
            if isinstance(override_cfg, str):
                return override_cfg
            try:
                resolved_overrides[name] = _resolve_delegation_credentials(override_cfg, parent_agent)
            except ValueError as exc:
                return tool_error(str(exc))
        per_task_creds = [
            resolved_overrides[name] if name else creds for name in override_names
        ]
    else:
        per_task_creds = [creds] * n_tasks
    header_creds = per_task_creds[0] if per_task_creds else (creds or {})
    ...
    children, err = _build_children(
        ...
        routing_cfg=routing_cfg, task_images=task_images,
    )
    if err:
        return tool_error(err)
    batch = _Batch(
        task_list, children, parent_agent, header_creds, context, top_role, max_children,
...
def _child_model_id(child) -> Optional[str]:
    """The child's post-construction model id, or None."""
...
def _picker_inventory_rows() -> List[dict]:
    try:
        ...
        payload = build_models_payload(
            load_picker_context(), for_picker=True, canonical_order=True,
            pricing=False, capabilities=False, featured=False,
            probe_custom_providers=False, probe_current_custom_provider=False,
            refresh=False, max_models=None,
        )
    except Exception:
        return []
...
def _extract_task_model_name(
    task: Dict[str, Any], top_level_model: str, task_index: int,
...
def _resolve_task_model_override(model_name: str, parent_agent):
    ...
    from hermes_cli.model_switch import get_authenticated_provider_slugs, switch_model
    ...
    try:
        result = switch_model(
            raw_input=name, current_provider=current_provider, current_model=current_model,
            current_base_url=current_base_url, current_api_key=current_api_key,
            is_global=False, explicit_provider="", user_providers=user_providers,
            custom_providers=custom_providers,
        )
    except Exception as exc:
        return tool_error(_format_unavailable_model_error(name, detail=str(exc)))
    if not getattr(result, "success", False):
        return tool_error(_format_unavailable_model_error(
            name, detail=getattr(result, "error_message", "") or "",
        ))
...
registry.register(
    ...
    handler=lambda args, **kw: delegate_task(
        ...
        model=args.get("model"), parent_agent=kw.get("parent_agent"),
    ),
