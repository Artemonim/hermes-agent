...
def _dispatch_background(batch: _Batch) -> str:
    ...
    child_models = [_child_model_id(c) for (_, _, c) in batch.children]
    named_child_models = [m for m in child_models if m]
    if named_child_models:
        batch_model = named_child_models[0]
    elif batch.creds is not None:
        batch_model = batch.creds.get("model")
    else:
        batch_model = None
    dispatch = dispatch_async_delegation_batch(
        ...
        role=batch.top_role, model=batch_model, models=child_models, session_key=session_key,
