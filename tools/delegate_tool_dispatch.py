...
def _dispatch_unit(unit: _Batch, unit_id: Optional[str], slot_key: Optional[str], routing: dict) -> dict:
    ...
    child_models = [_child_model_id(c) for (_, _, c) in unit.children]
    named_child_models = [m for m in child_models if m]
    if named_child_models:
        batch_model = named_child_models[0]
    elif unit.creds is not None:
        batch_model = unit.creds.get("model")
    else:
        batch_model = None
    return dispatch_async_delegation_batch(
        ...
        role=unit.top_role, model=batch_model, models=child_models,
