...
def _persist_dispatch(record: Dict[str, Any]) -> None:
    ...
    task_payload = {
        key: record.get(key)
        for key in ("goal", "goals", "context", "toolsets", "role", "model", "models", "is_batch", "task_indexes", *_ROUTING_KEYS)
        if key in record}
...
def dispatch_async_delegation_batch(
    *, goals: List[str], context: Optional[str], toolsets: Optional[List[str]], role: str, model: Optional[str],
    ...
    handle = _dispatch(
        ...
        toolsets=toolsets, role=role, model=model, models=models, session_key=session_key,
        parent_session_id=parent_session_id, runner=runner,
        origin_ui_session_id=origin_ui_session_id, origin_session_id=origin_session_id,
...
def _push_completion_event(record: Dict[str, Any], result: Dict[str, Any], status: str) -> None:
    ...
    evt = {
        ...
        **({"models": record.get("models")} if is_batch else {}),
    ...
    if is_batch:
        record_models = record.get("models") or []
        if isinstance(record_models, list):
            for row in evt.get("results") or []:
                if not isinstance(row, dict):
                    continue
                existing = row.get("model")
                if isinstance(existing, str) and existing.strip():
                    continue
                idx = row.get("task_index", 0)
                if not isinstance(idx, int) or idx < 0 or idx >= len(record_models):
                    continue
                stamped = record_models[idx]
