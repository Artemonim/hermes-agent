...
This checkout diverges from [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent)
on purpose. Fork-only features, weekly-merge hotspots, and the AE2 local CI
pipeline (`./run.ps1`) are recorded in [`FORK.md`](FORK.md) — not an upstream
Hermes contract. Do not dump fork doctrine back into this file; keep the
routing table and invariants above in sync with upstream's split `AGENTS.md`.
