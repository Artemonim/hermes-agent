...
import type { AccountUsageSnapshot, AccountUsageWindow, UsageStats } from '@/types/hermes'
...
export function formatCreditsBalance(value: number, locale: string): string {
            ...
            return (
              <li className="flex min-w-0 flex-col gap-1" key={window.label}>
                ...
                <div className="flex min-w-0 items-center justify-between gap-2 text-[0.6875rem] text-muted-foreground">
                  {used !== null && <span className="min-w-0 truncate">({copy.used(Math.round(used))})</span>}
                  <span className={cn('min-w-0 truncate', used !== null && 'shrink-0')}>
                    {Number.isFinite(resetAt) ? copy.resets(relativeTime(resetAt)) : (window.detail ?? '')}
                  </span>
                </div>
              </li>
            )
