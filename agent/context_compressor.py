...
def _content_length_for_budget(raw_content: Any) -> int:
    """Effective char-length of message content for budgeting: text by length plus image/audio equivalents."""
    if isinstance(raw_content, str):
        return len(raw_content)
    if not isinstance(raw_content, list):
        return len(str(raw_content or ""))
    # Any text-bearing part counts its text; image_url / input_audio payload size is irrelevant.
    total = 0
...
def _is_audio_part(part: Any) -> bool:
    """True if ``part`` is a native audio block."""
