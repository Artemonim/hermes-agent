...
def _strip_internal_ephemeral_keys(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Drop ``_hermes_ephemeral`` from the per-call copy so providers never see it.

    Live history keeps the tag for persist / end-of-turn strip; only the message
    dicts whose content actually changed are copied.
    """
    from agent.video_frame_extract import strip_ephemeral_metadata_from_content
    ...
    for msg in messages:
        if not isinstance(msg, dict):
            out.append(msg)
            continue
        cleaned = strip_ephemeral_metadata_from_content(msg.get("content"))
        if cleaned is msg.get("content"):
            out.append(msg)
            continue
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    substituted turn participates in the pairing and dedup passes. Internal ``_hermes_ephemeral``
    keys are stripped from the wire copy only."""
    ...
    return _strip_internal_ephemeral_keys(messages)
