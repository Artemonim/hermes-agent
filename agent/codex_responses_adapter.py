...
def _iter_content_parts(content: list) -> Iterator[tuple[str, Any]]:
    """Yield ``("text", str)`` / ``("image", part)`` / ``("audio", part)`` for recognized chat parts."""
    for part in content:
...
def _summarize_user_message_for_log(content: Any, *, sep: str = " ") -> str:
    ...
    providers feeding regexes), images → ``[N image(s)]`` marker, audio → ``[N audio]``, ``""`` for None,
    ...
    audio_count = sum(1 for kind, _ in parts if kind == "audio")
    notes = []
    if image_count:
        notes.append(f"[{image_count} image{'s' if image_count != 1 else ''}]")
    if audio_count:
        notes.append(f"[{audio_count} audio]")
    return " ".join(bit for bit in (*notes, sep.join(text_bits).strip()) if bit)
...
def _preflight_role_message(item: Dict[str, Any], idx: int, ctx: _PreflightCtx) -> Dict[str, Any]:
    ...
    for part_idx, part in enumerate(content):
        if isinstance(part, str):
            ...
            validated.append({"type": text_type, "text": "[audio]"})
        else:
            raise ValueError(
