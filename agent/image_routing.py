...
# Local path: same shape as gateway extract_local_files() — ``~/``, ``/``, or a
# Windows drive letter. Lookbehind skips matches inside URLs. URL: strict
# ``http(s)://`` so ``file://`` and other schemes are not grabbed; optional query.
...
def _existing_file(candidate: str) -> Optional[str]:
    ...
    try:
        if not os.path.isfile(expanded):
            return None
        # * expanduser("~/x") on Windows keeps the slash from "~/", so
        #   normalize before returning (callers compare against Path strings).
...
def _model_supports_capability_override(
    models_cfg: Any, model: str, primary_key: str, alias_key: str,
) -> Optional[bool]:
    """Per-model capability flag (``primary_key``, or ``alias_key``) from a ``models`` mapping."""
    per_model = _dict_or_empty(_dict_or_empty(models_cfg).get(model))
    return _coerce_capability_bool(per_model.get(primary_key, per_model.get(alias_key)))
...
def _model_supports_vision_override(models_cfg: Any, model: str) -> Optional[bool]:
    ...
    return _model_supports_capability_override(models_cfg, model, "supports_vision", "vision")
...
def _supports_capability_override(
    ...
    *,
    ...
    First hit wins: ``model.<primary_key>`` → ``providers.<p>.models.<model>``
    (``primary_key`` or ``alias_key``) → legacy ``custom_providers[].models``.
    Named custom providers are rewritten to ``provider="custom"`` at runtime
    while config keeps the user's name under ``model.provider``, so the
    requested, runtime and config identities are all tried, plus the bare
    ``<name>`` of any ``custom:<name>``. Returns None when unset.
    """
    ...
    return next(
        (v for v in (
            _model_supports_capability_override(m, model, primary_key, alias_key)
            for m in model_maps
        ) if v is not None),
        None,
    )
...
def _supports_vision_override(
    ...
    *,
    ...
    return _supports_capability_override(
        cfg, provider, model,
        primary_key="supports_vision", alias_key="vision",
