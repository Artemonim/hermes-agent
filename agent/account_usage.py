...
def serialize_account_usage_snapshot(snapshot: AccountUsageSnapshot) -> dict[str, Any]:
    ...
    Datetimes are ISO-8601 strings. The snapshot never contains the credential
    used to fetch it, so this is safe to send across the gateway to an
...
def _fetch_codex_account_usage(
    ...
    for key, label, label_key in (
        ...
        window = (payload.get("rate_limit") or {}).get(key) or {}
        used = window.get("used_percent")
        if used is None:
            continue
        windows.append(AccountUsageWindow(
            label=label, used_percent=float(used), reset_at=_parse_dt(window.get("reset_at")),
            label_key=label_key,
        ))
    details: list[str] = []
...
def _fetch_anthropic_account_usage(
    ...
    for key, label, label_key in (
        ...
        windows.append(AccountUsageWindow(
            label=label, used_percent=used, reset_at=_parse_dt(window.get("resets_at")),
            label_key=label_key,
        ))
    details: list[str] = []
    rows: list[AccountUsageRow] = []
    ...
    return _snapshot(
        "anthropic", "oauth_usage_api", windows, details,
        rows=tuple(rows), details_structured=True,
    )
...
def fetch_account_usage(
    ...
    if normalized in {"", "auto", "custom"} or normalized not in SUPPORTED_ACCOUNT_USAGE_PROVIDERS:
        return None
