...
are never mutated by this module. The wire overlay is the last step of
``agent.fast_mode.effective_request_overrides``; the ladder base is A1's
request-time ``logical_service_tier`` (session pin > per-model > global).

Accepted limitations (do not "fix"): non-streaming fallback produces no
observation; Codex passthrough parks ``on_first_delta`` and is not timed;
length-continuation and compression/redirect restarts drop the in-flight
sample (conservative under-escalation, self-heals on the next accepted
response). State is per-agent, never copied to children/aux, never persisted.
"""
...
def _surface_blocks_escalation(agent: Any) -> bool:
    ...
    if getattr(agent, "is_subagent", False):
        return True
    # * Hard gate for batch_runner and gateway/CLI/TUI background tasks: even
    # an accidentally-enabled config must not climb the ladder.
...
def bind_service_tier_escalation(agent: Any, raw_config: Any = None) -> ServiceTierEscalationState:
    ...
    state = ServiceTierEscalationState(
        config=config,
        base_tier=_logical_base_tier(agent),
    )
...
def begin_escalation_turn(agent: Any) -> None:
    """Entry of ``run_conversation``: always reset to the request-time base tier."""
    state = getattr(agent, "_service_tier_escalation", None)
    if not isinstance(state, ServiceTierEscalationState):
        return
    state.begin_turn(_logical_base_tier(agent))
...
def reset_escalation_for_model_switch(agent: Any) -> None:
    ...
    if not isinstance(state, ServiceTierEscalationState):
        return
    state.reset_for_model_switch(_logical_base_tier(agent))
