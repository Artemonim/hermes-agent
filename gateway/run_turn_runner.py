...
class TurnRunner:
    ...
    def _resolve_turn_agent(self, turn_route, platform_key, combined_ephemeral, max_iterations,
                            reasoning_config, pr):
    ...
    def _native_image_run_message(self):
        """Wrap the user turn as an OpenAI-style multimodal content list when inbound prep buffered
        image/audio/video-frame paths; consume-and-clear so later turns never re-attach stale media.
        Falls back to plain text when nothing is readable."""
        ctx = self._ctx

        def _paths(raw):
            return list(raw) if isinstance(raw, (list, tuple)) else []

        native_imgs = _paths(self._runner._consume_pending_native_image_paths(ctx.session_key))
        native_audio = _paths(self._runner._consume_pending_native_audio_paths(ctx.session_key))
        native_frames = _paths(self._runner._consume_pending_native_video_frame_paths(ctx.session_key))
        if not native_imgs and not native_audio and not native_frames:
            return ctx.message
        try:
            from agent.audio_routing import merge_native_media_parts
            parts, skipped_imgs, skipped_audio = merge_native_media_parts(
                ctx.message if isinstance(ctx.message, str) else "",
                native_imgs, native_audio, video_frame_paths=native_frames,
            )
            if skipped_imgs:
                logger.warning(
                    "Native image attachment: skipped %d unreadable path(s): %s",
                    len(skipped_imgs), skipped_imgs,
                )
            if skipped_audio:
                logger.info(
                    "Native audio attachment: skipped %d clip(s) (unreadable, silent, or over the 1-file cap): %s",
                    len(skipped_audio), skipped_audio,
                )
