...
class GatewayInboundMixin:
    ...
    @classmethod
    def _prepend_inbound_media_file_notes(
        cls, message_text: str, audio_file_paths: list[str], video_paths: list[str],
        video_frame_counts: Optional[Dict[str, int]] = None,
    ) -> str:
        ...
        for _path in video_paths:
            _display, _agent_path = cls._inbound_attachment_display_name(_path)
            _n_frames = int(video_frame_counts.get(_path) or 0)
            if _n_frames:
                note = (
    ...
    async def _prepare_inbound_message_text(
        ...
        @ references behave the same. Side effect: buffers per-session native image/audio/video-frame
        paths when the model supports native attach; the caller consumes those buffers at
        ``run_conversation``."""
        ...
        await self._stage_inbound_native_audio(
            source, session_key, audio_paths, audio_file_paths, video_paths,
        )
        video_frame_counts = await self._stage_inbound_video_frames(
            source, session_key, video_paths,
        )
        message_text = self._prepend_inbound_media_file_notes(
            message_text, audio_file_paths, video_paths, video_frame_counts=video_frame_counts,
        )
    ...
    async def _stage_inbound_native_audio(
        self, source: SessionSource, session_key: str,
        audio_paths: list[str], audio_file_paths: list[str], video_paths: list[str],
    ) -> None:
        """Stage voice notes, audio files, and video soundtracks for native attach when the model hears."""
        native_audio_candidates = list(dict.fromkeys([*audio_paths, *audio_file_paths, *video_paths]))
        if not native_audio_candidates:
            return
        ...
        if _audio_mode == "native":
            self._session_state(session_key).persistent.native_audio_paths = list(native_audio_candidates)
            logger.info(
                "Audio routing: native (model supports audio). %d clip(s) staged; at most one attaches inline.",
                len(native_audio_candidates),
            )
            return
        logger.info("Audio routing: text (mode=%s). STT / path notes only.", _audio_mode)

    async def _stage_inbound_video_frames(
        self, source: SessionSource, session_key: str, video_paths: list[str],
    ) -> Dict[str, int]:
        """Sample stills from inbound video when ``video.frame_extract`` is on and vision is native."""
        video_frame_counts: Dict[str, int] = {}
        if not video_paths:
            return video_frame_counts
        from agent.video_frame_extract import (
            extract_video_frames, frame_extract_is_enabled, max_frames_per_turn,
        )
        if not frame_extract_is_enabled():
            return video_frame_counts
        ...
        for vpath in video_paths:
            extracted = await asyncio.to_thread(extract_video_frames, vpath)
            video_frame_counts[vpath] = len(extracted)
        ...
        if staged_frames:
            self._session_state(session_key).persistent.native_video_frame_paths = list(staged_frames)
            logger.info(
                "Video frame extract: staged %d still(s) from %d video(s).",
                len(staged_frames), len(video_paths),
            )
        return video_frame_counts
    ...
    def _decide_audio_input_mode(
        ...
        """Resolve audio-input routing (``"native"`` / ``"text"``) for the effective model this turn
        (see agent/audio_routing.py). Mirrors ``_decide_image_input_mode`` so /model overrides apply
        to voice notes the same way they apply to photos."""
        try:
