...
| `/fast [normal\|fast\|status]` | Toggle fast mode — OpenAI Priority Processing / Anthropic Fast Mode. Options: `normal`, `fast`, `status`. `status` reports the effective service tier: `fast` (priority), `flex` (OpenRouter flex), or `normal`. |
...
| `/fast [normal\|fast\|status]` | Toggle fast mode — OpenAI Priority Processing / Anthropic Fast Mode. `status` reports the effective service tier: `fast` (priority), `flex` (OpenRouter flex), or `normal`. |
