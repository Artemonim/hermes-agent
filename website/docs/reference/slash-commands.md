...
| `/fast [normal\|fast\|flex\|auto\|cold\|status]` | Fast mode — OpenAI Priority Processing / Anthropic Fast Mode / OpenRouter flex. `fast` = every request; `flex` = OpenRouter Flex Processing; `auto` = only requests in the first `agent.fast_auto_seconds` (default 60s) of each turn; `cold` = that same window on the first turn of a session only. Default `normal` (off). `/fast` and `/fast status` report the **effective** tier for this session's model. See [Fast mode](../user-guide/configuration.md#fast-mode). |
...
| `/fast [normal\|fast\|flex\|auto\|cold\|status]` | Fast mode — OpenAI Priority Processing / Anthropic Fast Mode / OpenRouter flex. `auto`/`cold` open a bounded fast window per turn / per session. Status is ungated; switching to `fast` is route-gated. |
