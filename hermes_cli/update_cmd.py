...
def _log_only_write(text: str) -> None:
    ...
    Unwrapped stdout (gateway mode before hangup wrap, or a failed wrap) still appends
    ``logs/update.log`` so the Desktop watchdog sees progress.
    """
    if not text:
        return
    ...
    with suppress(Exception):
        ...
        with open(logs_dir / "update.log", "a", encoding="utf-8") as fh:
            fh.write(payload)


def _run_logged_subprocess(cmd, *, cwd=None, env=None):
    """Run ``cmd`` streaming combined output into update.log as it arrives.

    Buffering until ``subprocess.run`` returned made a long Desktop rebuild look
    stalled to the Windows idle watchdog. Returns the ``CompletedProcess`` so the
    caller can surface the output on failure.
    """
    chunks: list[str] = []
    ...
    try:
        ...
        with suppress(Exception):
            proc.wait(timeout=5)
        raise
    return subprocess.CompletedProcess(cmd, returncode, stdout="".join(chunks), stderr="")
...
def _is_shallow_checkout(git_cmd) -> bool:
    return _is_shallow_repository(git_cmd)


def _is_shallow_repository(git_cmd: list[str], cwd: Path | None = None) -> bool:
    """Return True when the checkout is shallow."""
    result = _git_run(git_cmd, ["rev-parse", "--is-shallow-repository"], cwd=cwd)
    return result.stdout.strip() == "true"


def _shallow_fetch_depth_args(git_cmd: list[str], cwd: Path | None = None) -> list[str]:
    """Return ``['--depth', '1']`` for a shallow checkout, else ``[]``."""
    if _is_shallow_repository(git_cmd, cwd):
        return ["--depth", "1"]
    return []
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        depth_args = _shallow_fetch_depth_args(git_cmd, _m().PROJECT_ROOT)
        fetch_result = _git_run(git_cmd, ["fetch"] + depth_args + ["origin", branch], network=True)
        if fetch_result.returncode != 0:
            _print_fetch_failure(fetch_result.stderr)
            sys.exit(1)
