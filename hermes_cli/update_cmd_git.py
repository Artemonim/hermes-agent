...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path, *, assume_yes: bool = False, input_fn=None) -> bool:
    ...
    from hermes_cli.update_cmd import (
        _count_commits_between, _has_upstream_remote, _no_prompt_git_kwargs,
        _shallow_fetch_depth_args, _should_skip_upstream_prompt)
    ...
    print("\n→ Fetching upstream...")
    try:
        depth_args = _shallow_fetch_depth_args(git_cmd, cwd)
        subprocess.run(
